...
class WordTypeCounter {
	public static void main(String[] args) throws Exception
	{
		if(args.length > 1)
		{
			// Declaracion e inicializaciun de variables.
			...
			// la implementacion SimpleSet que acompaoa esta tarea.
			...
			// Verificar que los dos parametros que se pasaron sean archivos que existen
			if(wordFile.isFile() && textFile.isFile()) {
				...
				while(line!=null)
				{
					wordParts = line.split("\\.");  // lo que esta entre comillas es una expresion regular.
				...
				while(line!=null)
				{
					// Separar todas las palabras en la linea.
					textParts = line.split("[^\\w-]+"); // utilizar de separador cualquier caracter que no sea una letra, numero o guion.
